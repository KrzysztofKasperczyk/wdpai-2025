...
class GameApiController extends AppController
{
    ...
    /**
     * POST /api/wheel/spin { session_id, options?[] }
     */
    public function wheelSpin(): void
    {
        $this->requireAuth();
        ...
        // sanitizacja prostym sposobem
        $options = array_values(array_filter(array_map(fn($x) => trim((string)$x), $options), fn($x) => $x !== ''));
        if (count($options) < 2) {
            $options = ['A', 'B', 'C', 'D'];
        }
        ...
        if ($session['game_type'] !== 'spin_wheel') {
            $this->json(['error' => 'wrong game type for this endpoint'], 400);
        }
        ...
        $eventId = $this->eventRepo->createEvent($sessionId, 'spin_wheel', $payload);

        $this->json(['ok' => true, 'event_id' => $eventId, 'result' => $result], 200);
    }
}
